<div class="flex flex-col justify-between w-full h-full">
    <div class="grid grid-cols-1 gap-12">
        ...
        <form method="POST" wire:submit="sendPasswordResetLink" class="col-span-1 flex flex-col gap-4">
            <label for="email" class="flex items-center gap-4 p-4 max-w-xl text-rose-quartz hover:text-green-500 @error('email') text-rose-500 @enderror bg-rose-quartz/10 hover:bg-white transition-all ease-in-out duration-300 cursor-pointer rounded-2xl">
                ...
                <div class="flex flex-col w-full">
                    ...
                    <input type="email" name="email" id="email" wire:model="email" required autofocus placeholder="example@example.com" class="w-full text-2xl text-dark-lavender placeholder:text-pale-dogwood font-semibold focus:outline-none focus:ring-0 focus:border-none"/>
                    @error('email')
            ...
            <button type="submit" class="flex items-center gap-4 p-4 max-w-xl min-h-24 text-sm text-white bg-light-lavender hover:bg-dark-lavender transition-all ease-in-out duration-300 cursor-pointer rounded-2xl">
                <div class="flex flex-col w-full text-left">
                    <h4 class="text-2xl font-semibold">{{ __('Email password reset link') }}</h4>
                    <span wire:loading.remove wire:target="sendPasswordResetLink" class="text-pale-dogwood">{{ __('We will send a reset link to your email') }}</span>
                    <span wire:loading wire:target="sendPasswordResetLink" class="text-pale-dogwood">{{ __('Sending...') }}</span>
                </div>
